...
def main():
    ...
    if not fanduel_matches:
        print(
            "No FanDuel odds found in the record. Please check the pattern and the source data."
        )
        # Handle the situation, e.g., by setting a default value or exiting the function
        df["Dexsports"] = [0] * len(df)  # Example default value
    else:
